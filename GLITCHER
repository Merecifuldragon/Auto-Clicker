-- ╔══════════════════════════════════════════════════════════════╗
-- ║  AUTO CLICKER by Merciful  v9                               ║
-- ║  H=Toggle  C=Lock  CTRL=Hide  L=Lag  B=Black               ║
-- ║  Tap the floating ⬡ button to show/hide the panel          ║
-- ╚══════════════════════════════════════════════════════════════╝
...
-- =====================================================================
-- EXECUTOR DETECTION  (Synapse, KRNL, Xeno, Fluxus, Delta…)
-- =====================================================================
local isXeno = (type(rawget(_G,"XENO")) == "table")
    or (identifyexecutor and string.find(string.lower(identifyexecutor()),"xeno"))
...
do
    ...
    if ok and h then
        guiParent = h
    elseif isXeno and typeof(rawget(_G,"XENO")) == "table" and XENO.GetGui then
        pcall(function() guiParent = XENO.GetGui() end)
    end
...
for _, n in ipairs({"AC_M_v9","AC_M_Marker_v9","AC_M_Toggle_v9"}) do
...
-- =====================================================================
-- CPU-OPTIMISED SHARED ANIMATION TICKER
-- All sine-wave UI animations share one RenderStepped accumulator
-- instead of running dozens of separate while-true loops.
-- =====================================================================
local animTick = 0
local animList = {}   -- {fn, offset}

local function addAnim(fn, offset)
    table.insert(animList, {fn=fn, o=offset or 0})
end

-- Only runs at ~20 fps for UI animations (not every render frame)
local animAccum = 0
RunService.RenderStepped:Connect(function(dt)
    animAccum = animAccum + dt
    animTick  = animTick  + dt
    -- Throttle UI animations to 20 fps — invisible difference, big CPU save
    if animAccum >= 0.05 then
        animAccum = 0
        for _, a in ipairs(animList) do
            pcall(a.fn, animTick + a.o)
        end
    end
end)
...
local HEAVY_MAT = {
    [Enum.Material.Grass]=true,[Enum.Material.Ground]=true,[Enum.Material.LeafyGrass]=true,
    [Enum.Material.Mud]=true,[Enum.Material.Sand]=true,[Enum.Material.Rock]=true,
    [Enum.Material.Cobblestone]=true,[Enum.Material.Brick]=true,[Enum.Material.Marble]=true,
    [Enum.Material.Granite]=true,[Enum.Material.Pebble]=true,[Enum.Material.Wood]=true,
    [Enum.Material.WoodPlanks]=true,[Enum.Material.Fabric]=true,[Enum.Material.Glass]=true,
    [Enum.Material.Foil]=true,[Enum.Material.Metal]=true,[Enum.Material.DiamondPlate]=true,
    [Enum.Material.Concrete]=true,[Enum.Material.Glacier]=true,[Enum.Material.Ice]=true,
    [Enum.Material.Basalt]=true,[Enum.Material.CrackedLava]=true,[Enum.Material.Limestone]=true,
    [Enum.Material.Pavement]=true,[Enum.Material.Salt]=true,[Enum.Material.Sandstone]=true,
...
local function sweepObj(obj)
    ...
    if obj:IsA("BasePart") then
        pcall(function()
            if HEAVY_MAT[obj.Material] then obj.Material = Enum.Material.SmoothPlastic end
    ...
    if obj:IsA("Terrain") then
        pcall(function()
            obj.WaterWaveSize=0; obj.WaterWaveSpeed=0
            obj.WaterReflectance=0; obj.WaterTransparency=0
        end)
    end
    for _, t in ipairs(SKY_TYPES) do
...
local function applyLag()
    ...
    pcall(function()
        ...
        if t then t.WaterWaveSize=0; t.WaterWaveSpeed=0; t.WaterReflectance=0 end
    end)
...
-- FPS limiter runs in its own connection separate from anim ticker
...
-- Resolve scale → pixels each frame (resize-safe)
RunService.RenderStepped:Connect(function()
...
-- =====================================================================
-- BLACK SCREEN  (ColorCorrectionEffect — blacks out 3D only, all UIs stay visible)
-- ON BY DEFAULT
-- =====================================================================
local blackEnabled = true

local existingCCE = Lighting:FindFirstChild("AC_BlackScreen_Effect")
if existingCCE then existingCCE:Destroy() end
...
blackEffect.Enabled    = true   -- ON by default
...
-- =====================================================================
-- SCREEN SIZE  — compact on mobile, wider on PC
-- =====================================================================
local VP       = workspace.CurrentCamera.ViewportSize
local IS_SMALL = VP.X < 600  -- phone/tablet
local W        = IS_SMALL and 310 or 340
local H        = IS_SMALL and 520 or 480
...
-- A small draggable pill always visible in the corner.
-- Tap/click it to show or hide the main panel.
-- Works on mobile and PC alike.
-- =====================================================================
local toggleGui = Instance.new("ScreenGui")
toggleGui.Name           = "AC_M_Toggle_v9"
toggleGui.ResetOnSpawn   = false
toggleGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
toggleGui.DisplayOrder   = 10000   -- above everything, always on top
toggleGui.IgnoreGuiInset = true
toggleGui.Parent         = guiParent

local PILL_W = isMobile and 70 or 62
local PILL_H = isMobile and 36 or 30

local pill = Instance.new("TextButton")
pill.Name              = "TogglePill"
pill.Size              = UDim2.new(0, PILL_W, 0, PILL_H)
pill.Position          = UDim2.new(0, 10, 0.5, -PILL_H/2)  -- left middle
pill.AnchorPoint       = Vector2.new(0, 0)
pill.BackgroundColor3  = C.cyanDk
pill.Text              = "⬡ AC"
pill.TextColor3        = C.cyan
...
pillStroke.Color     = C.cyan
pillStroke.Thickness = 1.8

-- Animate pill glow
addAnim(function(t)
    pillStroke.Transparency = 0.1 + (math.sin(t*1.4)+1)/2 * 0.55
end)

-- Pill drag (works on mobile touch AND PC mouse)
local pillDragging = false
local pillDragStart, pillStartPos

local function pillInputStart(pos)
    pillDragging  = true
    pillDragStart = pos
    pillStartPos  = pill.Position
end
local function pillInputMove(pos)
    if not pillDragging then return end
    local d = pos - pillDragStart
    pill.Position = UDim2.new(
        pillStartPos.X.Scale, pillStartPos.X.Offset + d.X,
        pillStartPos.Y.Scale, pillStartPos.Y.Offset + d.Y)
end
local function pillInputEnd(pos)
    if not pillDragging then return end
    pillDragging = false
    -- If the user barely moved it, treat as a tap/click
    local d = pos - pillDragStart
    if math.abs(d.X) < 6 and math.abs(d.Y) < 6 then
        gui.Enabled = not gui.Enabled
        pill.BackgroundColor3 = gui.Enabled and C.tabOn or C.cyanDk
        pill.TextColor3       = gui.Enabled and C.white or C.cyan
    end
end
...
pill.InputBegan:Connect(function(inp)
    ...
    or inp.UserInputType == Enum.UserInputType.Touch then
        pillInputStart(Vector2.new(inp.Position.X, inp.Position.Y))
    end
end)
UserInputService.InputChanged:Connect(function(inp)
    if inp.UserInputType == Enum.UserInputType.MouseMovement
    or inp.UserInputType == Enum.UserInputType.Touch then
        pillInputMove(Vector2.new(inp.Position.X, inp.Position.Y))
    end
end)
UserInputService.InputEnded:Connect(function(inp)
    if inp.UserInputType == Enum.UserInputType.MouseButton1
    or inp.UserInputType == Enum.UserInputType.Touch then
        pillInputEnd(Vector2.new(inp.Position.X, inp.Position.Y))
    end
end)

-- Start with pill showing panel is visible
pill.BackgroundColor3 = C.tabOn
...
-- =====================================================================
-- TITLE BAR  (62px, draggable)
-- =====================================================================
local TB_H = 62
local titleBar = Instance.new("Frame")
titleBar.Size=UDim2.new(1,0,0,TB_H); titleBar.BackgroundColor3=C.panel
...
tbFix.Position=UDim2.new(0,0,0.5,0); tbFix.BackgroundColor3=C.panel
tbFix.BorderSizePixel=0; tbFix.ZIndex=4; tbFix.Parent=titleBar

local titleLine=Instance.new("Frame"); titleLine.Size=UDim2.new(1,0,0,2)
titleLine.Position=UDim2.new(0,0,1,-2); titleLine.BackgroundColor3=C.cyan
titleLine.BorderSizePixel=0; titleLine.ZIndex=5; titleLine.Parent=titleBar

local accentBar=Instance.new("Frame"); accentBar.Size=UDim2.new(0,60,0,2)
accentBar.Position=UDim2.new(0,0,1,-4); accentBar.BackgroundColor3=C.mag
accentBar.BorderSizePixel=0; accentBar.ZIndex=6; accentBar.Parent=titleBar
...
-- Logo
local logoBox=Instance.new("Frame"); logoBox.Size=UDim2.new(0,44,0,44)
logoBox.Position=UDim2.new(0,10,0,9); logoBox.BackgroundColor3=C.cyanDk
logoBox.BorderSizePixel=0; logoBox.ZIndex=6; logoBox.Parent=titleBar
...
local lHex=Instance.new("TextLabel"); lHex.Size=UDim2.new(1,0,1,0)
lHex.BackgroundTransparency=1; lHex.Text="⬡"; lHex.TextColor3=C.cyan
lHex.TextScaled=true; lHex.ZIndex=7; lHex.Parent=logoBox
local lStar=Instance.new("TextLabel"); lStar.Size=UDim2.new(0.5,0,0.5,0)
lStar.Position=UDim2.new(0.25,0,0.25,0); lStar.BackgroundTransparency=1
lStar.Text="✦"; lStar.TextColor3=C.mag; lStar.TextScaled=true; lStar.ZIndex=8; lStar.Parent=logoBox

local titleTxt=Instance.new("TextLabel"); titleTxt.Size=UDim2.new(1,-64,0,24)
titleTxt.Position=UDim2.new(0,60,0,8); titleTxt.BackgroundTransparency=1
titleTxt.Text="AUTO CLICKER"; titleTxt.TextColor3=C.cyan; titleTxt.TextScaled=true
...
local subTxt=Instance.new("TextLabel"); subTxt.Size=UDim2.new(1,-64,0,14)
subTxt.Position=UDim2.new(0,60,0,34); subTxt.BackgroundTransparency=1
subTxt.Text="by Merciful · v9 · ⬡ pill = toggle panel"
...
for i=1,4 do
    local h=Instance.new("TextLabel"); h.Size=UDim2.new(0,18,0,18)
    h.Position=UDim2.new(1,-(i*20)-2,0,8); h.BackgroundTransparency=1; h.Text="⬡"
    h.TextColor3=C.cyan; h.TextTransparency=hexA[i]; h.TextScaled=true; h.ZIndex=5; h.Parent=titleBar
end
...
local TAB_Y  = TB_H
local TAB_H  = isMobile and 38 or 32
local tabBtns, tabPages, activeTab = {}, {}, 1
local tabW   = W/#TABS

local tabBar=Instance.new("Frame"); tabBar.Size=UDim2.new(1,0,0,TAB_H)
tabBar.Position=UDim2.new(0,0,0,TAB_Y); tabBar.BackgroundColor3=C.panel
tabBar.BorderSizePixel=0; tabBar.ZIndex=5; tabBar.Parent=frame
...
for i,name in ipairs(TABS) do
    ...
    local tline=Instance.new("Frame"); tline.Size=UDim2.new(1,0,0,2)
    ...
    page.ScrollBarImageColor3=C.cyanDim
    page.CanvasSize=UDim2.new(0,0,0,0)   -- will be updated per tab
...
for i=1,#TABS do
    local ci=i; tabBtns[i].btn.MouseButton1Click:Connect(function() switchTab(ci) end)
end

-- =====================================================================
-- UI HELPERS (scoped to scrolling page canvases)
-- =====================================================================
local function setCanvas(page, contentH)
    page.CanvasSize=UDim2.new(0,0,0,contentH)
end

local function mkCard(parent,y,h)
    local p=Instance.new("Frame"); p.Size=UDim2.new(1,-20,0,h); p.Position=UDim2.new(0,10,0,y)
...
local function mkLabel(par,x,y,w,h,txt,col,font,align)
    local l=Instance.new("TextLabel"); l.Size=UDim2.new(0,w,0,h); l.Position=UDim2.new(0,x,0,y)
...
local function mkInput(par,x,y,w,h,def,ph)
    local b=Instance.new("TextBox"); b.Size=UDim2.new(0,w,0,h); b.Position=UDim2.new(0,x,0,y)
...
local function mkGlowDot(par,x,y,col)
    local outer=Instance.new("Frame"); outer.Size=UDim2.new(0,14,0,14); outer.Position=UDim2.new(0,x,0,y)
    ...
    local inner=Instance.new("Frame"); inner.AnchorPoint=Vector2.new(0.5,0.5)
    inner.Size=UDim2.new(0,5,0,5); inner.Position=UDim2.new(0.5,0,0.5,0)
    inner.BackgroundColor3=col; inner.BorderSizePixel=0; inner.ZIndex=7; inner.Parent=outer
    Instance.new("UICorner",inner).CornerRadius=UDim.new(0.5,0)
    addAnim(function(t) stroke.Transparency=0.15+(math.sin(t)+1)/2*0.6 end, math.random(0,62)/10)
    return inner, stroke
end
...
local BTN_H = isMobile and 44 or 36   -- bigger tap targets on mobile

-- CPS card
local cpsCard=mkCard(pg1,8,isMobile and 100 or 90)

local cpsDispBg=Instance.new("Frame"); cpsDispBg.Size=UDim2.new(0,76,0,46)
cpsDispBg.Position=UDim2.new(0,8,0,8); cpsDispBg.BackgroundColor3=C.input
cpsDispBg.BorderSizePixel=0; cpsDispBg.ZIndex=5; cpsDispBg.Parent=cpsCard
...
addAnim(function(t) cpsDispStroke.Transparency=0.1+(math.sin(t)+1)/2*0.5 end, 1.2)

local cpsNum=Instance.new("TextLabel"); cpsNum.Size=UDim2.new(1,0,0.58,0)
cpsNum.Position=UDim2.new(0,0,0.04,0); cpsNum.BackgroundTransparency=1
cpsNum.TextColor3=C.cyan; cpsNum.TextScaled=true; cpsNum.Font=Enum.Font.GothamBold
cpsNum.ZIndex=6; cpsNum.Parent=cpsDispBg

local cpsUnit=Instance.new("TextLabel"); cpsUnit.Size=UDim2.new(1,0,0.35,0)
cpsUnit.Position=UDim2.new(0,0,0.63,0); cpsUnit.BackgroundTransparency=1
cpsUnit.Text="CPS"; cpsUnit.TextColor3=C.sub; cpsUnit.TextScaled=true
cpsUnit.Font=Enum.Font.Gotham; cpsUnit.ZIndex=6; cpsUnit.Parent=cpsDispBg

local function refreshCPS() cpsNum.Text=delaytoCPS(state.delay) end
refreshCPS()

-- Slider
local sliderW = W-40-92   -- responsive width
local sliderTrack=Instance.new("Frame"); sliderTrack.Size=UDim2.new(0,sliderW,0,isMobile and 10 or 7)
sliderTrack.Position=UDim2.new(0,92,0,30); sliderTrack.BackgroundColor3=C.input
sliderTrack.BorderSizePixel=0; sliderTrack.ZIndex=5; sliderTrack.Parent=cpsCard
...
local KNOB = isMobile and 22 or 18
local sKnob=Instance.new("TextButton"); sKnob.Size=UDim2.new(0,KNOB,0,KNOB)
sKnob.Position=UDim2.new(0.09,-KNOB/2,0.5,-KNOB/2); sKnob.BackgroundColor3=C.cyan
sKnob.Text=""; sKnob.BorderSizePixel=0; sKnob.ZIndex=7; sKnob.Parent=sliderTrack
...
local function applySliderPct(pct)
    ...
    sKnob.Position=UDim2.new(pct,-KNOB/2,0.5,-KNOB/2)
    refreshCPS()
end

local MINUS_X = 92+sliderW+4
mkBtn(cpsCard,"−",MINUS_X,10,isMobile and 32 or 26,isMobile and 38 or 30,C.cyanDim,5).MouseButton1Click:Connect(function()
    local cps=math.max(1,math.floor(1/state.delay)-10); state.delay=1/cps; applySliderPct((cps-1)/499)
end)
mkBtn(cpsCard,"+",MINUS_X+(isMobile and 36 or 30),10,isMobile and 32 or 26,isMobile and 38 or 30,C.cyanDim,5).MouseButton1Click:Connect(function()
...
sliderTrack.InputBegan:Connect(function(i)
    if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then
        sliderDrag=true; applySliderPct((i.Position.X-sliderTrack.AbsolutePosition.X)/sliderTrack.AbsoluteSize.X) end
end)
UserInputService.InputChanged:Connect(function(i)
    if sliderDrag and (i.UserInputType==Enum.UserInputType.MouseMovement or i.UserInputType==Enum.UserInputType.Touch) then
        applySliderPct((i.Position.X-sliderTrack.AbsolutePosition.X)/sliderTrack.AbsoluteSize.X) end
end)
...
-- Delay row
local CARD1_H = isMobile and 100 or 90
local delayRowY = CARD1_H-32
local delayBox=mkInput(cpsCard,92,delayRowY,90,isMobile and 28 or 22,"0.002","delay s")
mkLabel(cpsCard,8,delayRowY+2,82,18,"Delay(s):",C.sub)
mkBtn(cpsCard,"SET",186,delayRowY,52,isMobile and 28 or 22,C.cyanDim,5).MouseButton1Click:Connect(function()
...
-- Position lock card
local CARD1_BOT = CARD1_H+16
local posCard=mkCard(pg1,CARD1_BOT,isMobile and 96 or 84)
local lockDot,lockDotStroke=mkGlowDot(posCard,8,10,C.sub)
local lockInfo=mkLabel(posCard,28,8,W-60,18,"OFF — live cursor",C.sub)
mkLabel(posCard,8,28,W-28,14,"Hover target → tap LOCK",C.sub)

local LOCK_BTN_H = isMobile and 34 or 26
local LOCK_BTN_Y = (isMobile and 96 or 84)-LOCK_BTN_H-8
local lockBtn=mkBtn(posCard,"◈ LOCK  [C]",8,LOCK_BTN_Y,math.floor((W-36)/2),LOCK_BTN_H,C.cyanDim,5)
local unlockBtn=mkBtn(posCard,"◎ UNLOCK",math.floor((W-36)/2)+12,LOCK_BTN_Y,math.floor((W-36)/2),LOCK_BTN_H,C.red,5)
...
local function applyLock()
    ...
    lockInfo.TextColor3=C.cyan; lockDot.BackgroundColor3=C.cyan; lockDotStroke.Color=C.cyan
...
-- Status strip
local CARD2_BOT = CARD1_BOT+(isMobile and 96 or 84)+10
local statCard=mkCard(pg1,CARD2_BOT,isMobile and 36 or 30)
local statusDot=mkGlowDot(statCard,8,8,C.sub)
local statusLbl=mkLabel(statCard,28,5,W-50,20,"IDLE · Tap START",C.sub)

-- Mobile quick-action buttons (H / C / L / B as big tap targets)
-- On PC these are just keyboard shortcuts; on mobile we need visible buttons
local MOBILE_BTN_Y = CARD2_BOT+(isMobile and 36 or 30)+10
local mobileRowH   = isMobile and 44 or 0   -- hidden on PC (keys work fine)
if isMobile then
    local mBW = math.floor((W-30)/4)
    local mbLabels = {"H\nClick","C\nLock","L\nLag","B\nBlack"}
    local mbCols   = {C.green, C.cyanDim, C.orange, C.sub}
    for i=1,4 do
        mkBtn(pg1,mbLabels[i],10+(i-1)*(mBW+2),MOBILE_BTN_Y,mBW,mobileRowH,mbCols[i],5)
        .MouseButton1Click:Connect(function()
            if i==1 then
                -- handled below in setActive toggle
                local newState = not state.clicking
                setActive(newState)   -- forward declaration; will be defined shortly
            elseif i==2 then
...
-- START button
local START_Y = MOBILE_BTN_Y + mobileRowH + (mobileRowH>0 and 8 or 0)
local startBtn=mkBtn(pg1,"▶  START  [H]",10,START_Y,W-40,BTN_H,C.green,5)

local shimmer=Instance.new("Frame"); shimmer.Size=UDim2.new(0,36,1,0)
shimmer.Position=UDim2.new(0,-36,0,0); shimmer.BackgroundColor3=Color3.fromRGB(255,255,255)
shimmer.BackgroundTransparency=0.82; shimmer.BorderSizePixel=0; shimmer.ZIndex=6; shimmer.Parent=startBtn
...
-- Forward-declare setActive so mobile buttons above can call it
local setActive
setActive = function(on)
    ...
    if on then
        ...
        statusLbl.Text="IDLE · Tap START"; statusLbl.TextColor3=C.sub; statusDot.BackgroundColor3=C.sub
    end
end
startBtn.MouseButton1Click:Connect(function() setActive(not state.clicking) end)

-- Fix the mobile H button now that setActive exists
-- (already connected via the forward ref above — no extra work needed)

local TAB1_CANVAS_H = START_Y + BTN_H + 16
setCanvas(pg1, TAB1_CANVAS_H)
...
local function mkBadge(par,x,y,w,h,txt,col,fn)
    ...
    local lbl=Instance.new("TextLabel"); lbl.Size=UDim2.new(1,0,1,-6); lbl.Position=UDim2.new(0,0,0,5)
...
-- Responsive badge sizing — 4 badges in a row
local bW=math.floor((W-50)/4)
local bH=isMobile and 66 or 58
local bGap=math.floor((W-20-bW*4)/3)
...
local function refreshFpsBtn()
    if fpsLocked then
        ...
        fpsBadgeLbl.TextColor3=C.cyan; fpsBadgeStroke.Color=C.cyan
    else
        fpsBadgeLbl.Text="🔓FPS\nFREE"
        fpsBadgeLbl.TextColor3=C.sub; fpsBadgeStroke.Color=C.sub
    end
end
...
blkLbl,blkStroke=mkBadge(pg2,bx(4),8,bW,bH,"",C.green,function()   -- starts ON/green
    setBlack(not blackEnabled); refreshBlk()
end); refreshBlk()

-- FPS CAP card
local FPS_CARD_Y = bH+18
local fpsCapCard=mkCard(pg2,FPS_CARD_Y,isMobile and 120 or 106)
mkLabel(fpsCapCard,8,6,80,16,"FPS CAP",C.cyan,Enum.Font.GothamBold)

local presets={10,20,30,60,120,240}
local pW2=math.floor((W-40)/#presets)
for i,fps in ipairs(presets) do
    local pb=mkBtn(fpsCapCard,tostring(fps),8+(i-1)*pW2,24,pW2-2,isMobile and 28 or 22,C.cyanDk,5)
    pb.MouseButton1Click:Connect(function()
        setFpsTarget(fps); fpsLocked=true; lastFrame=tick(); refreshFpsBtn()
        local pct=(fpsTarget-1)/239
        fFill2.Size=UDim2.new(pct,0,1,0); fKnob2.Position=UDim2.new(pct,-KNOB/2,0.5,-KNOB/2)
        fpsNum2.Text=tostring(fpsTarget); fpsIn2.Text=tostring(fpsTarget)
    end)
end

local PRESET_BOT = 24+(isMobile and 28 or 22)+6
local fpsDispBg2=Instance.new("Frame"); fpsDispBg2.Size=UDim2.new(0,52,0,34)
fpsDispBg2.Position=UDim2.new(0,8,0,PRESET_BOT); fpsDispBg2.BackgroundColor3=C.input
fpsDispBg2.BorderSizePixel=0; fpsDispBg2.ZIndex=5; fpsDispBg2.Parent=fpsCapCard
...
addAnim(function(t) fBStroke2.Transparency=0.1+(math.sin(t)+1)/2*0.5 end, 2.0)

local fpsNum2=Instance.new("TextLabel"); fpsNum2.Size=UDim2.new(1,0,0.6,0)
fpsNum2.Position=UDim2.new(0,0,0.04,0); fpsNum2.BackgroundTransparency=1
fpsNum2.Text=tostring(fpsTarget); fpsNum2.TextColor3=C.cyan
fpsNum2.TextScaled=true; fpsNum2.Font=Enum.Font.GothamBold; fpsNum2.ZIndex=6; fpsNum2.Parent=fpsDispBg2

local fpsUnit2=Instance.new("TextLabel"); fpsUnit2.Size=UDim2.new(1,0,0.35,0)
fpsUnit2.Position=UDim2.new(0,0,0.63,0); fpsUnit2.BackgroundTransparency=1
fpsUnit2.Text="FPS"; fpsUnit2.TextColor3=C.sub; fpsUnit2.TextScaled=true
fpsUnit2.Font=Enum.Font.Gotham; fpsUnit2.ZIndex=6; fpsUnit2.Parent=fpsDispBg2

local FPS_SLIDER_W = W-40-68
local fTrack2=Instance.new("Frame"); fTrack2.Size=UDim2.new(0,FPS_SLIDER_W,0,isMobile and 10 or 7)
fTrack2.Position=UDim2.new(0,66,0,PRESET_BOT+12)
...
local fKnob2=Instance.new("TextButton"); fKnob2.Size=UDim2.new(0,KNOB,0,KNOB)
fKnob2.Position=UDim2.new((fpsTarget-1)/239,-KNOB/2,0.5,-KNOB/2)
...
local FPS_IN_Y = PRESET_BOT+36
local fpsIn2=mkInput(fpsCapCard,66,FPS_IN_Y,58,isMobile and 28 or 20,tostring(fpsTarget),"FPS")
mkBtn(fpsCapCard,"SET",128,FPS_IN_Y,46,isMobile and 28 or 20,C.cyanDim,5).MouseButton1Click:Connect(function()
...
local FPS_CTRL_X = 66+FPS_SLIDER_W+6
mkBtn(fpsCapCard,"−",FPS_CTRL_X,PRESET_BOT,isMobile and 28 or 22,isMobile and 46 or 38,C.cyanDk,5).MouseButton1Click:Connect(function()
    local pct=(math.max(1,fpsTarget-1)-1)/239; setFpsTarget(math.max(1,fpsTarget-1))
    fpsLocked=true; lastFrame=tick(); fFill2.Size=UDim2.new(pct,0,1,0); fKnob2.Position=UDim2.new(pct,-KNOB/2,0.5,-KNOB/2)
    fpsNum2.Text=tostring(fpsTarget); fpsIn2.Text=tostring(fpsTarget); refreshFpsBtn()
end)
mkBtn(fpsCapCard,"+",FPS_CTRL_X+(isMobile and 32 or 26),PRESET_BOT,isMobile and 28 or 22,isMobile and 46 or 38,C.cyan,5).MouseButton1Click:Connect(function()
    local pct=(math.min(240,fpsTarget+1)-1)/239; setFpsTarget(math.min(240,fpsTarget+1))
    fpsLocked=true; lastFrame=tick(); fFill2.Size=UDim2.new(pct,0,1,0); fKnob2.Position=UDim2.new(pct,-KNOB/2,0.5,-KNOB/2)
    fpsNum2.Text=tostring(fpsTarget); fpsIn2.Text=tostring(fpsTarget); refreshFpsBtn()
end)
...
fTrack2.InputBegan:Connect(function(i)
    if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then
        fDrag2=true; local p=math.clamp((i.Position.X-fTrack2.AbsolutePosition.X)/fTrack2.AbsoluteSize.X,0,1)
        local f=math.clamp(math.round(1+p*239),1,240); setFpsTarget(f); fpsLocked=true; lastFrame=tick()
        fFill2.Size=UDim2.new(p,0,1,0); fKnob2.Position=UDim2.new(p,-KNOB/2,0.5,-KNOB/2)
...
UserInputService.InputChanged:Connect(function(i)
    if fDrag2 and (i.UserInputType==Enum.UserInputType.MouseMovement or i.UserInputType==Enum.UserInputType.Touch) then
        local p=math.clamp((i.Position.X-fTrack2.AbsolutePosition.X)/fTrack2.AbsoluteSize.X,0,1)
        local f=math.clamp(math.round(1+p*239),1,240); setFpsTarget(f); fpsLocked=true; lastFrame=tick()
        fFill2.Size=UDim2.new(p,0,1,0); fKnob2.Position=UDim2.new(p,-KNOB/2,0.5,-KNOB/2)
        fpsNum2.Text=tostring(fpsTarget); fpsIn2.Text=tostring(fpsTarget); refreshFpsBtn() end end)
UserInputService.InputEnded:Connect(function(i)
...
local TAB2_H = FPS_CARD_Y+(isMobile and 120 or 106)+16
setCanvas(pg2, TAB2_H)
...
local lagToggle=mkBtn(pg3,"⚡ LAG REDUCTION: ON  [L]",10,8,W-40,isMobile and 44 or 34,C.green,5)
...
local COL_COUNT = isMobile and 1 or 2
local colW2=math.floor((W-30)/COL_COUNT)
local LAG_TOGGLE_H = isMobile and 44 or 34
local OPTS_START_Y = LAG_TOGGLE_H+18

for i,opt in ipairs(opts) do
    local col_=((i-1)%COL_COUNT); local row_=math.floor((i-1)/COL_COUNT)
    local bx2_=10+col_*(colW2+5)
    local CB_H = isMobile and 44 or 32
    local by2_=OPTS_START_Y+row_*(CB_H+6)
    local on=LagSettings[opt.k]
    local cb=Instance.new("TextButton"); cb.Size=UDim2.new(0,colW2,0,CB_H)
    cb.Position=UDim2.new(0,bx2_,0,by2_); cb.BackgroundColor3=C.card
    cb.Text=""; cb.BorderSizePixel=0; cb.AutoButtonColor=false; cb.ZIndex=5; cb.Parent=pg3
    ...
    local cbIcon=Instance.new("TextLabel"); cbIcon.Size=UDim2.new(0,28,1,0); cbIcon.Position=UDim2.new(0,6,0,0)
    ...
    local cbLbl=Instance.new("TextLabel"); cbLbl.Size=UDim2.new(1,-36,1,0); cbLbl.Position=UDim2.new(0,34,0,0)
...
local ROW_COUNT = math.ceil(#opts/COL_COUNT)
local CB_H_TOTAL = isMobile and 44 or 32
local OPTS_END_Y = OPTS_START_Y + ROW_COUNT*(CB_H_TOTAL+6) + 10

local infoLbl=mkLabel(pg3,10,OPTS_END_Y,W-30,30,
    "GC every 30s · Sky removed · Textures/mesh/grass stripped",C.sub)
infoLbl.TextWrapped=true; infoLbl.TextScaled=false; infoLbl.TextSize=isMobile and 12 or 10

setCanvas(pg3, OPTS_END_Y+40)

-- =====================================================================
-- LOCK MARKER OVERLAY
-- =====================================================================
local markerGui=Instance.new("ScreenGui"); markerGui.Name="AC_M_Marker_v9"
markerGui.ResetOnSpawn=false; markerGui.ZIndexBehavior=Enum.ZIndexBehavior.Sibling
markerGui.IgnoreGuiInset=true; markerGui.DisplayOrder=5000; markerGui.Parent=guiParent
...
RunService.RenderStepped:Connect(function()
    if not state.useFixedPos then
        ring.Visible=false; mDot.Visible=false; lH2.Visible=false; lV2.Visible=false; return
    end
...
-- =====================================================================
-- KEYBOARD BINDS  (PC only — mobile uses on-screen buttons)
-- =====================================================================
...
print("✅ Auto Clicker by Merciful v9 | ⬡ pill = toggle panel | H/C/L/B | Mobile ready")
